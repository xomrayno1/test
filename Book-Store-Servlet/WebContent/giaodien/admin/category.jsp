	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-10">
					...
					<c:forEach items="${listCategory}" var="item" varStatus="i">
						<tr>
					...
					<tr>
						<td colspan="3">
							...
							<li><a href='<c:url value="/admin/category?page=${pre}"></c:url>'>&laquo;</a>
								<c:forEach begin="1"  end="${soPage}" var="item">
									<c:if test="${pageCurrent != item}">
										<li  ><a href='<c:url value="/admin/category?page=${item}"></c:url>'>${item}</a></li>
									</c:if>
									<c:if test="${pageCurrent == item}">
										<li class="active" ><a href='<c:url value="/admin/category?page=${item}"></c:url>'>${item}</a></li>
									</c:if>
								...
								<li><a href='<c:url value="/admin/category?page=${next}"></c:url>'>&raquo;</a>
							</ul>
						</td>
					</tr>
					<tr>
						<td colspan="3"><button data-toggle="modal" data-target="#mymodal" class="btn btn-primary">Thêm</button></td>
					</tr>
				</table>
			</div>			
		</div>
