		...
		<div class="row">
			...
			<div class="col-sm-10">
				<div class="panel panel-primary">
					<div class="panel-heading">Sách danh mục</div>
					<div class="panel-body">
						<div class="row text-center">
							<c:forEach items="${listP}" var="item">
								...
								<c:set value="${item.category_Id}" var="idcategory" />
							</c:forEach>
						</div>
					...
					<div class="panel-footer text-center">
						...
						<ul class="pagination">
							...
							<li><a href='<c:url value="/category?id=${idcategory}&&page=${pre}"></c:url>'>&laquo;</a></li>

							<c:forEach begin="1" end="${soPage}" var="vl">							
								<c:if test="${pageCurrent != vl}">
									<li ><a  href='<c:url value="/category?id=${idcategory}&&page=${vl}"></c:url>'>${vl}</a></li>
								</c:if>
								<c:if test="${pageCurrent == vl}">
									<li class="active"><a  href='<c:url value="/category?id=${idcategory}&&page=${vl}"></c:url>'>${vl}</a></li>
								</c:if>
							</c:forEach>
							...
							<li><a href='<c:url value="/category?id=${idcategory}&&page=${next}"></c:url>'>&raquo;</a></li>
				...
				<div class="panel panel-primary panel-category">
					...
					<div class="panel-body">
						<div class="nav flex-column nav-pills nav-category">
							<a href="#" class="nav-link">Tâm lý - Kỹ năng sống</a> <a
								href="#" class="nav-link">Kinh tế quản lý</a> <a href="#"
								class="nav-link">Marketing - Quản Lý Bán Hàng</a>
						</div>
				...
				<div class="panel-body">
					...
					<c:forEach items="${listBooksViewed}" var="item">
						<div class="col-sm-3">
							<div class="thumbnail" style="height: 284px;">								
									<img src='<c:url value="${item.images}"></c:url>'>
									<p><a href='<c:url value="/product-detail?id=${item.id}"></c:url>'>${item.title}</a></p>
